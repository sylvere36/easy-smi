...
@RoutePage()
class ProfileSensibilizationPage extends StatelessWidget {
  static const String routeName = '/profile-sensibilization';
  const ProfileSensibilizationPage({super.key});

  @override
  Widget build(BuildContext context) {
    return const MyScaffold(
      appBarTitle: 'SENSIBILISATION',
      paddingHorizontale: 0,
      body: _SensibilizationProofBody(),
    );
  }
}

class _SensibilizationProofBody extends StatefulWidget {
  const _SensibilizationProofBody();

  @override
  State<_SensibilizationProofBody> createState() =>
      _SensibilizationProofBodyState();
}

class _SensibilizationProofBodyState extends State<_SensibilizationProofBody> {
  // UI palette
  ...
  // Form state
  String? _sensibilization;
  String _pieceJointe = 'Piece jointe';
  DateTime? _participation = DateTime(2025, 10, 09);

  // Data sources (sensibilization list)
  final List<String> _sensibilizations = const [
    'Sensibilization A',
    'Sensibilization B',
    'Sensibilization C',
    'Sensibilization D',
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _DropdownField(
                    label: _sensibilization ?? 'Selectionnez la compétence',
                    hintColor: _sensibilization == null ? kHint : kLabel,
                    radius: kRadius,
                    fill: kFieldFill,
                    stroke: kStroke,
                    onTap: () {
                      setState(() {
                        _sensibilization =
                            _sensibilizations[Random.secure().nextInt(4)];
                      });
                      context.router.push(
                        const ProfileSearchSensibilizationRoute(),
                      );
                    },
                  ),

                  // -------- Attestation -----------
                  _label('Preuve', top: 22),
                  _AttachmentField(
                    text: _pieceJointe,
                    onTapChevron: () {
                  ...
                  // -------- Date (two columns) -----------
                  _label('Date de participation', top: 22),
                  _DateField(
